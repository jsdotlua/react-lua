...
-- local type {ReactPortal} = require(Workspace.Shared.ReactTypes)
...
-- local {emptyRefsObject} = require(script.Parent.ReactFiberClassComponent.new)
...
-- local {StrictMode} = require(script.Parent.ReactTypeOfMode)
...
-- local warnForMissingKey = (child: mixed, returnFiber: Fiber) => {}
...
--     if child == nil or typeof child ~= "table")
--       return
--     end
--     if !child._store or child._store.validated or child.key ~= nil)
--       return
--     end
--     invariant(
...
--     typeof mixedRef ~= "table"
--   )
--     if __DEV__)
--       -- TODO: Clean this up once we turn on the string ref warning for
--       -- everyone, because the strict mode case will no longer be relevant
--       if 
--         (returnFiber.mode & StrictMode or warnAboutStringRefs) and
...
--     if element._owner)
--       local owner: ?Fiber = (element._owner: any)
--       local inst
--       if owner)
--         local ownerFiber = ((owner: any): Fiber)
--         invariant(
...
local function ChildReconciler(shouldTrackSideEffects)
  ...
  --   -- instead.
  --   local existingChildren: Map<string | number, Fiber> = new Map()

  --   local existingChild = currentFirstChild
  --   while (existingChild ~= nil)
  --     if existingChild.key ~= nil)
  ...
  --   lanes: Lanes,
  -- )
  --   if current == nil or current.tag ~= HostText)
  --     -- Insert
  ...
  --   key: nil | string,
  -- ): Fiber {
  --   if current == nil or current.tag ~= Fragment)
  --     -- Insert
  ...
  --   lanes: Lanes,
  -- ): Fiber | nil {
  --   if typeof newChild == 'string' or typeof newChild == 'number')
  ...
  --   if typeof newChild == "table" and newChild ~= nil)
  ...
  --     if isArray(newChild) or getIteratorFn(newChild))
  --       local created = createFiberFromFragment(
  --         newChild,
  --         returnFiber.mode,
  --         lanes,
  ...
  --   local key = oldFiber ~= nil ? oldFiber.key : nil
  ...
  --     local matchedFiber = existingChildren.get(newIdx) or nil
  ...
  --             newChild.key == nil ? newIdx : newChild.key,
  --           ) or nil
  --         if newChild.type == REACT_FRAGMENT_TYPE)
  --           return updateFragment(
  --             returnFiber,
  ...
  -- ): Set<string> | nil {
  --   if __DEV__)
  --     if typeof child ~= "table" or child == nil)
  --       return knownKeys
  --     end
  --     switch (child.$$typeof)
  --       case REACT_ELEMENT_TYPE:
  ...
  --     -- First, validate keys.
  --     local knownKeys = nil
  --     for (local i = 0; i < newChildren.length; i++)
  --       local child = newChildren[i]
  ...
  --   for (; oldFiber ~= nil and newIdx < newChildren.length; newIdx++)
  ...
  --     end
  --     if shouldTrackSideEffects)
  --       if oldFiber and newFiber.alternate == nil)
  ...
  --     if newFiber ~= nil)
  --       if shouldTrackSideEffects)
  --         if newFiber.alternate ~= nil)
  --           -- The new fiber is a work in progress, but if there exists a
  --           -- current, that means that we reused the fiber. We need to delete
  --           -- it from the child list so that we don't add it to the deletion
  --           -- list.
  --           existingChildren.delete(
  --             newFiber.key == nil ? newIdx : newFiber.key,
  --           )
  --         end
  --       end
  --       lastPlacedIndex = placeChild(newFiber, lastPlacedIndex, newIdx)
  ...
  --     local newChildren = iteratorFn.call(newChildrenIterable)
  --     if newChildren)
  --       local knownKeys = nil
  ...
  --   -- way to define them.
  --   if currentFirstChild ~= nil and currentFirstChild.tag == HostText)
  ...
  local function reconcileSingleElement(
      ...
      if child.key == key then
        if child.tag == Fragment then
          if element.type == REACT_FRAGMENT_TYPE then
            unimplemented("reconcileSingleElement: Fragment")
        ...
        elseif child.tag == Block then
          ...
          then
            unimplemented("Hot reloading logic")
            -- deleteRemainingChildren(returnFiber, child.sibling)
    ...
    if element.type == REACT_FRAGMENT_TYPE then
      ...
      local created = createFiberFromElement(element, returnFiber.mode, lanes)
      warn("Skip unimplemented: ReactChildFiber.coerceRef")
  ...
  local function reconcileChildFibers(
    ...
    if typeof(newChild) == "string" or typeof(newChild) == "number" then
      unimplemented("string or number child")
      -- return placeSingleChild(
    ...
    if getIteratorFn(newChild) then
      unimplemented("children iterator")
    ...
    if _G.__DEV__ then
      if typeof(newChild) == "function" then
        unimplemented("warnOnFunctionType")
        -- warnOnFunctionType(returnFiber)
      end
    end
