...
describe("ReactIncremental", function()
	...
	it("can deprioritize unfinished work and resume it later", function()
		...
		expect(Scheduler).toFlushAndYield({ "Foo", "Bar", "Bar", "Middle", "Middle" }) -- Render part of the work. This should be enough to flush everything except
		-- the middle which has lower priority.
		ReactNoop:render(React.createElement(Foo, { text = "bar" }))
		expect(Scheduler).toFlushAndYieldThrough({ "Foo", "Bar", "Bar" }) -- Flush only the remaining work
	...
	it("can deprioritize a tree from without dropping work", function()
		...
		expect(Scheduler).toFlushAndYield({ "Middle", "Middle" }) -- Render the high priority work (everything except the hidden trees).
		ReactNoop:flushSync(function()
			ReactNoop:render(React.createElement(Foo, { text = "foo" }))
		end)
		expect(Scheduler).toHaveYielded({ "Foo", "Bar", "Bar" }) -- The hidden content was deprioritized from high to low priority. A low
	...
	xit("can resume work in a subtree even when a parent bails out", function()
		...
		local middleContent = React.createElement(
			"aaa",
			nil,
			React.createElement(Tester, nil),
			React.createElement(
				"bbb",
				{ hidden = true },
		...
		expect(Scheduler).toHaveYielded({ "Foo", "Bar", "Tester", "Bar" }) -- We're now rendering an update that will bail out on updating middle.
		ReactNoop:render(React.createElement(Foo, { text = "bar" }))
		ReactNoop:flushDeferredPri(45 + 5)
		expect(Scheduler).toHaveYielded({ "Foo", "Bar", "Bar" }) -- Flush the rest to make sure that the bailout didn't block this work.
	...
	xit("can resume work in a bailed subtree within one pass", function()
		...
		type Tester = React_Component<any, any> & {}
		type Tester_statics = {}
		local Tester = React.Component:extend("Tester") :: Tester & Tester_statics
		function Tester.shouldComponentUpdate(self: Tester)
			return false
		end
		function Tester.render(self: Tester)
			-- This component is just here to ensure that the bail out is
			-- in fact in effect in the expected place for this test.
			Scheduler:unstable_yieldValue("Tester")
			return React.createElement("div", nil)
		...
		local function Middle(props)
			Scheduler:unstable_yieldValue("Middle")
			return React.createElement("span", nil, props.children)
		end -- Should content not just bail out on current, not workInProgress?
		type Content = React_Component<any, any> & {}
		type Content_statics = {}
		local Content = React.Component:extend("Content") :: Content & Content_statics
		function Content.shouldComponentUpdate(self: Content)
			return false
		end
