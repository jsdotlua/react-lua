 ...
 * some cases. Better to just block all updates in initialization.
 ...
 return function()
    ...
    it("should warn if using old Roact didMount naming", function()
        ...
        end).toWarnDev("The method name 'didMount' is no longer supported and should be updated to 'componentDidMount'", {withoutStack = true})
    end)
    it("should warn if using old Roact shouldUpdate naming", function()
        local Foo = React.Component:extend("Foo")
        jestExpect(function()
            function Foo:shouldUpdate() end
        end).toWarnDev("The method name 'shouldUpdate' is no longer supported and should be updated to 'shouldComponentUpdate'", {withoutStack = true})
    end)
    it("should warn if using old Roact willUpdate naming", function()
        local Foo = React.Component:extend("Foo")
        jestExpect(function()
            function Foo:willUpdate() end
        end).toWarnDev("The method name 'willUpdate' is no longer supported and should be updated to 'UNSAFE_componentWillUpdate'", {withoutStack = true})
    end)
    it("should warn if using old Roact didUpdate naming", function()
        local Foo = React.Component:extend("Foo")
        jestExpect(function()
            function Foo:didUpdate() end
        end).toWarnDev("The method name 'didUpdate' is no longer supported and should be updated to 'componentDidUpdate'", {withoutStack = true})
    end)
    it("should warn if using old Roact willUnmount naming", function()
        local Foo = React.Component:extend("Foo")
        jestExpect(function()
            function Foo:willUnmount() end
        end).toWarnDev("The method name 'willUnmount' is no longer supported and should be updated to 'componentWillUnmount'", {withoutStack = true})
    end)
