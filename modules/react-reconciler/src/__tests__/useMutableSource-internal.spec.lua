	...
	describe("useMutableSource", function()
		...
		local function Component(props)
			...
			return React.createElement(
				"div",
				nil,
				string.format("%s:%s", label, snapshot)
			)
		end
		...
		it(
			"should re-use previously read snapshot value when reading is unsafe",
			function()
				local source = createSource("one")
				local mutableSource = createMutableSource(source, function(param)
					return param.version
		...
		it(
			...
			function()
				...
				local function App(props)
					...
					local first = useMutableSource(
						mutableSource,
						getSnapshotFirst,
						defaultSubscribe
					)
					local second = useMutableSource(
						mutableSource,
						getSnapshotSecond,
						defaultSubscribe
					)
		...
		it(
			...
			function()
				...
				local function App(props)
					local parentConfig, childConfig =
						props.parentConfig, props.childConfig
		...
		it(
			...
			function()
				...
				local function ComponentA()
					local snapshot = useMutableSource(
						mutableSource,
						defaultGetSnapshot,
						defaultSubscribe
					)
