return function()
	...
	describe("ReactFiberTreeReflection", function()
		...
		describe("getSuspenseInstanceFromFiber", function()
			...
			it(
				"returns the dehydrated memoized state from the alternate fiber",
				function()
					local suspenseInstance = {}
					local fiber = mockFiber({
						alternate = mockFiber({
			...
			it(
				"returns null if the fiber does not have the SuspenseComponent tag",
				function()
		...
		describe("different fiber states", function()
			...
			local function generateIsFiberMounted(expected)
				...
				it(string.format("isFiberMounted() is %s", tostring(expected)), function()
					jestExpect(ReactFiberTreeReflection.isFiberMounted(fiber)).toBe(
						expected
					)
				end)
			end
			...
			local function generateIsMounted(expected)
				...
				it(string.format("isMounted() is %s", tostring(expected)), function()
					...
					jestExpect(ReactFiberTreeReflection.isMounted(component)).toBe(
						expected
					)
				end)
			end
				...
				for name, flag in fiberFlags :: any do
					describe(
						...
						function()
							...
							it(
								"getNearestMountedFiber() returns the parent fiber",
								function()
									jestExpect(
										ReactFiberTreeReflection.getNearestMountedFiber(
											fiber
										)
									).toBe(rootFiber)
								end
		...
		describe("doesFiberContain", function()
			...
			it(
				"is true if the parent alternate and the child are the same fiber",
				function()
					local fiber = mockFiber()
