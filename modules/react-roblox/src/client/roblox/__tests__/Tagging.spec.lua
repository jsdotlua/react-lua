	...
	if _G.__DEV__ then
		describe("adding tags", function()
			...
			beforeEach(function()
				...
				tag1AddedConnection = CollectionService
					:GetInstanceAddedSignal("tag1")
					:Connect(tag1Fn)
				tag2AddedConnection = CollectionService
					:GetInstanceAddedSignal("tag2")
					:Connect(tag2Fn)
		...
		describe("removing tags", function()
			...
			beforeEach(function()
				local tag1Mock, tag1Fn = jest.fn()
				local tag2Mock, tag2Fn = jest.fn()
				tag1RemovedConnection = CollectionService
					:GetInstanceRemovedSignal("tag1")
					:Connect(tag1Fn)
				tag2RemovedConnection = CollectionService
					:GetInstanceRemovedSignal("tag2")
					:Connect(tag2Fn)
