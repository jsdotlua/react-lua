...
return function()
    describe('ReactProfiler DevTools integration', function()
        ...
        it('should auto-Profile all fibers if the DevTools hook is detected', function()
            ...
            local rendered = ReactTestRenderer.create(React.createElement(App, {multiplier = 1}))
            ...
            jestExpect(onRender).toHaveBeenCalledWith('Profiler', 'mount', 10, 10, 2, 12, Set.new())
            onRender.mockClear()
            ...
            jestExpect(rendered.root:findByType(App):_currentFiber().treeBaseDuration).toBe(12)
            ...
            -- But needs to be accounted for in the offset times.
            jestExpect(onRender).toHaveBeenCalledTimes(1)
            jestExpect(onRender).toHaveBeenCalledWith('Profiler', 'update', 6, 13, 14, 20, Set.new())

            -- Measure unobservable timing required by the DevTools profiler.
            -- At this point, the base time should include both:
            -- The initial 9ms for the components that do not re-render, and
            -- The updated 6ms for the component that does.
            jestExpect(rendered.root:findByType(App):_currentFiber().treeBaseDuration).toBe(15)
        end)
        ...
        it('should reset the fiber stack correctly after an error when profiling host roots', function()
            ...
            rendered.update(React.createElement('div', nil, React.createElement(AdvanceTime, {byAmount = 7})))
            ...
            jestExpect(rendered.root:findByType('div'):_currentFiber().treeBaseDuration).toBe(7)
        end)
