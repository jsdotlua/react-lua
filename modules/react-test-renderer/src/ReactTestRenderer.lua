...
toTree = function(nodeInput: Fiber | nil)
    ...
    elseif node.tag == Fragment or 
        ...
        node.tag == Mode or
        node.tag == Profiler or
        node.tag == ForwardRef or
        node.tag == MemoComponent or
        node.tag == IncompleteClassComponent or
        node.tag == ScopeComponent then
      return childrenToTree(node.child);
    else
      invariant(
        false,
        'toTree() does not yet know how to handle nodes with tag=' .. tostring(node.tag)
      )
    end
    return
end
...
local function findAll(
    root: Object,
    predicate: Predicate,
    options: FindOptions?): Array<Object>
...
function ReactTestInstance:_currentFiber() : Fiber
    ...
    invariant(fiber ~= nil, "Can't read from currently-mounting component. This error is likely " ..
...
function ReactTestInstance.new(fiber: Fiber)
    invariant(validWrapperTypes[fiber.tag] ~= nil, 'Unexpected object passed to ReactTestInstance constructor (tag: %s). ' ..
...
function ReactTestInstance:find(predicate: Predicate): Object
    return expectOne(self:findAll(predicate, {deep = false}), ('matching custom predicate: %s'):format(tostring(predicate)))
end
function ReactTestInstance:findByType(type_: any): Object
    return expectOne(self:findAllByType(type_, {deep = false}), ('with node type: "%s"'):format(getComponentName(type_) or 'Unknown'))
end
function ReactTestInstance:findByProps(props: Object): Object
    return expectOne(self:findAllByProps(props, {deep = false}), ('with props: %s'):format(JSON:JSONEncode(props)))
end
...
local function create(element, options: TestRendererOptions)
    ...
    local entry = {
        ...
        toJSON = function()
            ...
            if #container.children == 2 and container.children[1].isHidden == true and container.children[2].isHidden == false then
