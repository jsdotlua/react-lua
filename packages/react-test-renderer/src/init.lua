...
-- local getInstancesForTag = require(script.Parent .. ComponentProps).getInstancesForTag
...
--  TODO: split below to silence analyze, but why is analyze throwing in first place?
local ReactTestHostConfig = require(script.ReactTestHostConfig)
--  deviation: For all tests, we mock the reconciler into a configurable
...
--  deviation: add type for Array and Object
type Array<T> = { [number]: T }
type Object = { [string]: any }

type TestRendererOptions = {
	--  TODO: upstream treats this as optional, somehow flowtype doesn't complain
...
local function toJSON(inst)
	...
	--  deviation: if/else instead of switch
...
--  deviation: toTree needs to be pre-declared to avoid function call cycle
local toTree
...
--  deviation: change node variable name to nodeInput so we can keep the node name
...
toTree = function(nodeInput: Fiber | nil)
	...
	--  deviation: silence analyze by recasting
	local node: any = nodeInput

	--  deviation: swtich converted to if/else
	...
	elseif node.tag == ClassComponent then
		return {
			nodeType = "component",
			type = node.type,
			--  deviation: Uses Object.assign for shallow copy
...
--  TODO: port ReactTestInstance type infered from the upstream class declaration
local ReactTestInstance = {}

--  deviation: not using Set()
...
local function getChildren(parent)
	...
	--  deviation: use break flag instead of labeled loops
...
local function findAll(root: Object, predicate: Predicate, options: FindOptions?): Array<Object>
	--  deviation: ternary split to conditional statement
	local deep = true
	if options then
		deep = options.deep
	end
	...
	--  deviation: use for loop instead of forEach
	for _, child in root.children do
		if typeof(child) == "string" then
			continue
		end
		--  deviation: use for loop to insert mulltiple elements
...
--  deviation:  metatable includes upstream
...
function ReactTestInstance.new(fiber: Fiber)
	...
	--  deviation: set metatable to ReactTestInstanceGetters which includes upstream
	-- getter methods and Class methods
...
--  deviation START: the first argument gets an explicit nil in many tests
local function create(element: ReactElement<any, any> | nil, options: TestRendererOptions?)
	...
	--  deviation: remove Fiber? type to silence analyze
...
function flushActWork(resolve, reject)
	...
	--  TODO: Jest runONlyPendingTimers() not implemented (uncomment line below)
