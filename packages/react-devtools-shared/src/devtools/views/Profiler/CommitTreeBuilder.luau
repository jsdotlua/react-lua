...
local TREE_OPERATION_UPDATE_TREE_BASE_DURATION = constantsModule.TREE_OPERATION_UPDATE_TREE_BASE_DURATION
	...
	while i <= #operations do
		...
		if operation == TREE_OPERATION_ADD then
			...
			if nodes:has(id) then
				error("Commit tree already contains fiber " .. tostring(id) .. ". This is a bug in React DevTools.")
			end
		...
		elseif operation == TREE_OPERATION_REMOVE then
			...
			for _ = 1, removeLength do
				...
				if not nodes:has(id) then
					error("Commit tree does not contain fiber " .. tostring(id) .. ". This is a bug in React DevTools.")
				end
				...
				if not nodes:has(parentID) then
					...
					local parentNode = getClonedNode(parentID)
					if __DEBUG__ then
						debug_("Remove", ("fiber %s from parent %s"):format(tostring(id), tostring(parentID)))
					end
					parentNode.children = Array.filter(parentNode.children, function(childID)
						return childID ~= id
					end)
				end
			end
		elseif operation == TREE_OPERATION_REORDER_CHILDREN then
			...
			local children = (Array.slice(operations, i, i + numChildren) :: any) :: Array<number>
			i += numChildren
			if __DEBUG__ then
		...
		elseif operation == TREE_OPERATION_UPDATE_TREE_BASE_DURATION then
			...
			if __DEBUG__ then
				debug_(
					"Update",
					("fiber %s treeBaseDuration to %s"):format(tostring(id), tostring(node.treeBaseDuration))
				)
			end
		else
			error(string.format("Unsupported Bridge operation %s at operation index %d", tostring(operation), i))
		end
	end
...
local function getCommitTree(ref: {
	...
	local commitIndex, profilerStore, rootID = ref.commitIndex, ref.profilerStore, ref.rootID
	...
	if commitIndex == 1 then
		...
		recursivelyInitializeTree(rootID, 0, nodes, dataForRoot :: ProfilingDataForRootFrontend) -- Mutate the tree
		if operations ~= nil and commitIndex <= #operations then
			local commitTree = updateTree({ nodes = nodes, rootID = rootID }, operations[commitIndex])
			if __DEBUG__ then
