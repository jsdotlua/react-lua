...
local PROFILER_EXPORT_VERSION = require(script.Parent.Parent.Parent.Parent.constants).PROFILER_EXPORT_VERSION
	...
	for _, dataBackend in dataBackends do
		for _, ref in dataBackend.dataForRoots do
			...
			if operations == nil then
				error(Error.new(string.format("Could not find profiling operations for root %s", tostring(rootID))))
			end
			local snapshots = snapshotsByRootID:get(rootID)
			if snapshots == nil then
				error(Error.new(string.format("Could not find profiling snapshots for root %s", tostring(rootID))))
			end
			...
			local convertedCommitData = Array.map(commitData, function(commitDataBackend, commitIndex)
				return {
					changeDescriptions = if commitDataBackend.changeDescriptions ~= nil
						then Map.new(commitDataBackend.changeDescriptions)
						else nil,
					duration = commitDataBackend.duration,
					fiberActualDurations = Map.new(commitDataBackend.fiberActualDurations),
					fiberSelfDurations = Map.new(commitDataBackend.fiberSelfDurations),
...
exports.prepareProfilingDataFrontendFromBackendAndStore = prepareProfilingDataFrontendFromBackendAndStore
local function prepareProfilingDataFrontendFromExport(profilingDataExport: ProfilingDataExport): ProfilingDataFrontend
	local version_ = profilingDataExport.version
	if version_ ~= PROFILER_EXPORT_VERSION then
		error(string.format('Unsupported profiler export version "%s"', tostring(version_)))
	end
	local dataForRoots: Map<number, ProfilingDataForRootFrontend> = Map.new()
		...
		dataForRoots:set(rootID, {
			commitData = Array.map(commitData, function(ref)
				local changeDescriptions, duration, fiberActualDurations, fiberSelfDurations, interactionIDs, priorityLevel, timestamp =
					...
					ref.timestamp
				return {
					changeDescriptions = if changeDescriptions ~= nil then Map.new(changeDescriptions) else nil,
					duration = duration,
...
local function prepareProfilingDataExport(profilingDataFrontend: ProfilingDataFrontend): ProfilingDataExport
...
local function scale(minValue: number, maxValue: number, minRange: number, maxRange: number)
