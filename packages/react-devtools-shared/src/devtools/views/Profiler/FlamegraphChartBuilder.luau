...
local function getChartData(ref: {
	...
	local fiberActualDurations, fiberSelfDurations = commitDatum.fiberActualDurations, commitDatum.fiberSelfDurations
	...
	local function walkTree(id: number, rightOffset: number, currentDepth: number)
		...
		local maybeKey = if Boolean.toJSBoolean(key) then (' key="%s"'):format(tostring(key)) else ""
	...
	if nodes.size > 0 then
		...
		if root == nil then
			error(string.format('Could not find root node with id "%s" in commit tree', tostring(rootID)))
		end
