...
def display(file, root, title, colors, flip = False):
    ...
    for n in root.subtree():
        ...
        file.writelines([
            "<g>",
            "<title>{}</title>".format(escape(n.title())),
            "<details>{}</details>".format(escape(n.details(root))),
            "<rect x='{}' y='{}' width='{}' height='{}' fill='{}' rx='2' ry='2' />".format(x, y, width, height, fill),
            "<text x='{}' y='{}'>{}</text>".format(x + 3, y + 10.5, escape(text)),
            "<rawtext>{}</rawtext>".format(escape(n.text())),
            "</g>",
        ])
